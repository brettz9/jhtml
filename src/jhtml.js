...
*/
let SAJJ, ObjectArrayDelegator, Stringifier;
...
function item2JSONObject (item, allowJS, throwOnSpan) {
  ...
  case 'i': // null, boolean, number (or undefined, function, non-finite number, Date or RegExp object)
    ...
    default:
      ...
      // function
      const funcMatch = textContent.match(/^function \w*\(([\w, ]*)\) \{([\s\S]*)\}$/);
      if (funcMatch) {
      ...
      // RegExp
      const regexMatch = textContent.match(/^\/([\s\S]*)\/(g?)(i?)(m?)$/);
      if (regexMatch) {
        const flags = (regexMatch[2] ? 'g' : '') + (regexMatch[3] ? 'i' : '') + (regexMatch[4] ? 'm' : '');
...
const JHTMLStringifier = SAJJ.createAndReturn({
...
exp.toJSONObject = function (items, options) {
  options = options || {};
  const jsonHtml = (items && items.nodeType === 1) ? [items] : (items || document.getItems(jhtmlNs));
  const ret = [].map.call(jsonHtml, function (item) {
...
exp.toJHTMLDOM = function (jsonObj) {
  const jhtmlStr = this.toJHTMLString(jsonObj);
