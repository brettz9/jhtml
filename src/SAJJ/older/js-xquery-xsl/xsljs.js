...
export default function ProcessTemplates (ts) {
  this.templates = ts;
  this.templateParams = {};
  const unsortedMatches = [];
  for (const template in ts) { // Don't filter with hasOwnProperty() as may wish to inherit
    let matchPattern = template;
    ...
    if (template.match(/^\s/)) { // ' /  mode=Hello priority=5 name=Root'
      const pattern = template.match(/\s*(\S*)\s*(.*)/);
      const params = pattern[2].split(/\s+/);
      matchPattern = pattern[1];
      for (let i = 0, pl = params.length; i < pl; i++) {
    ...
    while (treeWalker.nextNode()) {
      console.log(treeWalker.currentNode); // Todo: use it
    }
  }
  ts.$apply = function () {
...
const jsonHTMLTemplates = {
  // Pass in 'that'-like obj reference as 2nd argument for convenience
