...
function ignoreHarmlessNonelementNodes(node, item) {
  if ((node.nodeType === 3 || node.nodeType === 4) && // Text or CDATA node
...
function item2JSONObject(item, allowJS, throwOnSpan) {
  ...
  switch (topLevelJSONElement) {
    ...
    case 'i':
      // null, boolean, number (or undefined, function, non-finite number, Date or RegExp object)
      switch (textContent) {
        case 'null':
          return null;
      ...
      if (!allowJS) {
        throw new Error('The value type (' + ret + ') cannot be used in JSON mode');
      }
    ...
    case 'ol':
      ...
      ret = []; // JSON allows empty arrays (and HTML allows empty <ol>'s) so we do also
...
var exp;
var jhtmlNs = 'http://brett-zamir.me/ns/microdata/json-as-html/2';
var JHTMLStringifier = SAJJ.createAndReturn({
  ...
  methods: {
    ...
    objectKeyHandler: function objectKeyHandler(key, parentObject, parentKey, parentObjectArrayBool, iterCt) {
      return '<dt>' + escapeHTMLText(key) + '</dt>';
    },
    arrayKeyHandler: function arrayKeyHandler(key, parentObject, parentKey, parentObjectArrayBool) {
      return '';
    },
    ...
    beginHandler: function beginHandler(obj, parObj, parKey, parObjArrBool) {
    ...
    endHandler: function endHandler(obj, parObj, parKey, parObjArrBool) {
      var objType = _typeof(obj);
